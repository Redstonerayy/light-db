...
void configure_server_context(SSL_CTX *ctx)
{
    ...
    if (SSL_CTX_use_certificate_chain_file(ctx, "./tls/cert.pem") <= 0) {
        ERR_print_errors_fp(stderr);
        exit(EXIT_FAILURE);
    }

    if (SSL_CTX_use_PrivateKey_file(ctx, "./tls/key.pem", SSL_FILETYPE_PEM) <= 0) {
        ERR_print_errors_fp(stderr);
        exit(EXIT_FAILURE);
    }
}
...
void configure_client_context(SSL_CTX *ctx)
{
     ...
     */
    if (!SSL_CTX_load_verify_locations(ctx, "./tls/cert.pem", NULL)) {
        ERR_print_errors_fp(stderr);
        exit(EXIT_FAILURE);
    }
}
