...
Schüler Freundschaften und Kursbelegungen hat. Diese Testdaten generierte ich mir
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=3cm,
        ...
        \node[main node] (1) {S 1};
        \node[main node] (2) [below left of=1] {S 2};
        \node[main node] (3) [below right of=1] {K 1};

        \path[every node/.style={font=\sffamily\small}]
        (1) edge [bend right] node[left] {Freund} (2)
        (2) edge node [right] {Freund} (1)
        edge node {Kurs} (3)
        (1) edge [bend left] node[right] {Kurs} (3);
    \end{tikzpicture}
...
Das $H$ beschreibt die Suche nach allen Kursbelegungen des Schülers Nr. 1.
